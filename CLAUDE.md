...
- [ ] **Provenance tracking — shipped, awaiting end-to-end verification.** Code landed in commits `d3d43e3` (plan) + `7909cdb` (implementation). Five new columns on `projects`: `front_image_source`, `back_image_source`, `left_image_source`, `right_image_source`, `generator`. Schema migration in `init-db.js` is idempotent and includes a one-time backfill (`UPDATE … WHERE generator IS NULL AND model_file IS NOT NULL`) that stamped existing completed rows with `'hitem3dv2.0'`. Backend auto-stamps `generator` in `runHitem3dJob`, image sources on `/api/upload-front` and `/api/upload-view` (from a `source` body field), and `<view>_image_source = "openai_gpt-image-1.5"` on `/api/generate-view`. PATCH `/api/projects/:id` allow-list extended to accept the four `*_image_source` fields (normalized to lowercase). `createVariant` copies image sources from parent but NOT `generator`. Frontend has a per-card Source dropdown (Midjourney / OpenAI / Other…) on Step 2 with localStorage pre-fill, and the history card status label now reads `"Model · HiTem3D"` when a generator is set. Plan and step-by-step test plan in [PROVENANCE-PLAN.md](PROVENANCE-PLAN.md) §7. Resume by running through that test plan; mark this `[x]` once verified.
