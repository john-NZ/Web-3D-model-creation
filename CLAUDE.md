...
- [ ] Add Remove/Delete button to each card in the History strip on Step 1, so the user can clean up abandoned/unwanted projects. Backend already has `DELETE /api/projects/:id` (no backend work). Frontend: small "×" or trash icon in the corner of each `.history-card`; click → `confirm("Delete project #X?")` → DELETE → `loadHistory()` to refresh. If the deleted project is the currently-loaded one (`currentProjectId === deletedId`), also clear local state (similar to btnStartOver: viewImages, modelFilename, banners) and stay on Step 1. Note: schema uses `ON DELETE SET NULL` on `projects.parent_id`, so any variants of the deleted project survive with an orphaned `parent_id`; the lineage badge in their card will still show "↳ #N" but loading them won't break — out of scope to handle the orphaned reference more gracefully right now.
